...
// Same resumable/TUS path as uploadAudioFile, scoped under a per-user demo
// prefix so track + voice-note pairs never collide across artists.
async function uploadDemoFile(file: File, userId: string, kind: 'track' | 'voice', title: string, onProgress?: (pct: number) => void): Promise<string> {
  const ext = (file.name.split('.').pop() ?? 'mp3').toLowerCase();
  const safeName = title.replace(/[^a-z0-9]/gi, '_').toLowerCase();
  const path = `demos/${userId}/${kind}_${Date.now()}_${safeName}.${ext}`;
  const EXT_MIME: Record<string, string> = {
    mp3: 'audio/mpeg', wav: 'audio/wav', flac: 'audio/flac',
    aac: 'audio/aac', m4a: 'audio/mp4', ogg: 'audio/ogg', oga: 'audio/ogg',
  };
...
export function uploadDemoTrack(file: File, userId: string, title: string, onProgress?: (pct: number) => void): Promise<string> {
  return uploadDemoFile(file, userId, 'track', title, onProgress);
}

export function uploadDemoVoiceNote(file: File, userId: string, title: string, onProgress?: (pct: number) => void): Promise<string> {
  return uploadDemoFile(file, userId, 'voice', title, onProgress);
}
