      ...
      .then(({ data }) => {
        ...
        if (data) {
          supabase
            ...
            .eq('city_id', data.id)
            .then(({ data: rows }) =>
              setSceneArtists((rows ?? []).map(r => r.artists).filter(Boolean))
            );
        }
      });
  }, [slug]);
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-12">
            ...
            <section>
              ...
              <p className="text-slate-300 leading-relaxed">{scene.long_description}</p>
            </section>

            {/* Genres */}
            {(scene.genre_stats ?? []).length > 0 && (
              <section>
                <h2 className="text-xl font-bold text-white mb-5">Genreverdeling</h2>
                <div className="bg-white/3 border border-white/8 rounded-2xl p-6 space-y-4">
                  {scene.genre_stats.map(g => (
