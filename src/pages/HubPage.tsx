...
import {
  ...
  Clock, ChevronRight, Loader2, Eye, MessageSquare, Pencil, Trash2, Mail, ArrowUpRight,
} from 'lucide-react';
...
// 'forums' and 'network' are covered in full by the dedicated /forums and
// /netwerken pages (reachable from the Community dropdown) — those tabs
// just link out instead of re-rendering a second, thinner copy here.
// 'spaces' has no standalone equivalent (map + on-site reviews), so it
// stays inline.
const SECTIONS = [
  { id: 'forums',  label: 'Forums',   icon: MessageCircle, desc: 'Vragen, tips & discussies over muziek maken', route: '/forums' },
  { id: 'spaces',  label: 'Ruimtes',  icon: Home,          desc: 'Repetitieruimtes, studio\'s & jamspaces' },
  { id: 'network', label: 'Netwerken', icon: Users,         desc: 'Samenwerking, collabs & open calls', route: '/netwerken' },
];
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-6">
          <div className="flex gap-1 -mb-px overflow-x-auto scrollbar-hide">
            {SECTIONS.map(section => {
              ...
              // Forums & Netwerken are full features of their own (see
              // /forums, /netwerken) — this tab bar just links out to them
              // instead of rendering a second, thinner copy inline.
              if (section.route) {
                return (
                  <Link
                    key={section.id}
                    to={section.route}
                    className="flex items-center gap-2 px-4 py-3.5 text-sm font-medium whitespace-nowrap border-b-2 border-transparent text-slate-500 hover:text-slate-300 transition-all"
                  >
                    <Icon size={15} />
                    {section.label}
